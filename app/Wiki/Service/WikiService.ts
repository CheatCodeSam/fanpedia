...
export default class WikiService {
	public static async getWikiBySubdomains(
		...
		if (!wiki) throw new WikiNotFoundException('Wiki cannot been found')
		view.share({ wiki: wiki })
		return wiki
	}

	public static async getWikiBySlug(
		slug: string
	): Promise<WikiMetadata | null> {
		const wikiResult = await g.V().has('wiki', 'slug', slug).elementMap().next()
