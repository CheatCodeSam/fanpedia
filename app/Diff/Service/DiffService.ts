...
export default class DiffService {
	private static mergeBlocks(
		...
		const highc = Math.max(
			using[0][using[0].length - 1].endA,
			using[1][using[1].length - 1].endA
		)
	...
	public static twoWayDiff(a: string[], b: string[]): Changes {
		return DiffService.threeWayDiff(a, b, b)
	}
	...
	public static threeWayDiff(a: string[], o: string[], b: string[]): Changes {
		...
		while (seq0.length || seq1.length) {
			...
			while (seq[otherSeq].length > 0) {
				const otherDiff = seq[otherSeq][0]
			...
			if (using[0].length === 0) {
				...
				if (
					h0 - l0 === h2 - l2 &&
					a.slice(l0, h0).join('') === b.slice(l2, h2).join('')
				) {
					if (l1 !== h1 && l0 === h0) {
						tag = 'delete'
					} else if (l1 !== h1) {
