	...
	public async validateToken(token: string, claim: TokenClaim) {
		...
		if (decodedToken.payload.token_use !== claim)
			throw new jose.errors.JWTClaimValidationFailed(
				`Missing required "${claim}" token_use.`
			)

		if (decodedToken.payload.token_use === 'id')
			if (decodedToken.payload.aud !== config.client_id)
				throw new jose.errors.JWTInvalid()
		if (decodedToken.payload.token_use === 'access')
			if (decodedToken.payload.client_id !== config.client_id)
				throw new jose.errors.JWTInvalid()
